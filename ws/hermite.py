...
def point_matrix(P1,DC1,DC2,theta):
    ...
    T1 = np.array([np.cos(np.radians(45)),-x*np.cos(np.radians(theta))*np.sin(np.radians(45)),-x*np.sin(np.radians(theta))*np.sin(np.radians(45))])
    T2 = np.array([-np.cos(np.radians(45)),-x*np.cos(np.radians(theta))*np.sin(np.radians(45)),-x*np.sin(np.radians(theta))*np.sin(np.radians(45))])
...
def hermite(theta):
    for u in np.arange(0,1,0.01):
        ...
        ax.text(0, 0, 1.1, r'$z$')
        x=5
        ax.plot3D([0,x*np.cos(np.radians(45))],[0, -x * np.cos(np.radians(90-theta)) * np.cos(np.radians(45))],[0,-x * np.sin(np.radians(90-theta)) * np.cos(np.radians(45))],'b')
